...
- **TabPFN v2** (2025 tabular foundation model) — _rejected_: v8 gates weight download
  behind a Prior-Labs license + `TABPFN_TOKEN` (external credential), caps context ~10k rows.
- **GBM / XGBoost / LightGBM** — _excluded by constraint_ (ensembles/boosting). Kept only
